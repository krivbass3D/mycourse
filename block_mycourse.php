...
class block_mycourse extends block_base {
    ...
    public function init(): void {
        ...
        $this->title = get_string('plugindisplayname', 'block_mycourse');
    ...
    public function specialization(): void {
        ...
        if(!empty($this->config->title)) {
            ...
            $this->title = get_string('title', 'block_mycourse');
        }
        ...
        // update block title
            ...
            if($this->config->filter_course_group == 2) {
                $this->title = get_string('title_not_subscribed', 'block_mycourse');
            }

            else {
                $this->title = get_string('title_subscribed', 'block_mycourse');
            }
        }
            ...
            switch($this->config->filter_completion) {
                ...
                case 1:
                    $this->title = get_string('title_completed', 'block_mycourse');
                    break;
                case 2:
                case 3:
                    $this->title = get_string('title_uncompleted', 'block_mycourse');
                    break;
            }
        }
    ...
    public function get_required_javascript(): void {
        ...
        if($this->config->display == 2 || ($this->config->alt_display == 2 && $this->config->show_altview)) {
            $this->page->requires->js_call_amd('block_mycourse/menu','init',['id' => $this->instance->id]);
        }
        if($this->config->display != 2 || ($this->config->alt_display != 2 && $this->config->show_altview)) {
            $this->page->requires->js_call_amd('block_mycourse/show_more','init',['id' => $this->instance->id]);
        }

        if($this->config->show_categories == 1 || ($this->config->alt_show_categories == 1 && $this->config->show_altview == 1)) {
                 $this->page->requires->js_call_amd('block_mycourse/collapse', 'init', ['id' => $this->instance->id]);
        }

        if($this->config->show_altview) {
            $this->page->requires->js_call_amd('block_mycourse/switch_view','init',['id' => $this->instance->id]);
        }

        if($this->config->show_favorites && block_mycourse\helper::favorites_installed()) {
        ...
        if($this->config->course_slider && $this->config->top_category != 0) {
            $this->page->requires->js_call_amd('block_mycourse/top_category_slider_init','init',['id' => $this->instance->id, 'category' => $this->config->top_category, 'current_view' => 0]);
        }

        if($this->config->alt_course_slider && $this->config->alt_top_category != 0) {
            $this->page->requires->js_call_amd('block_mycourse/top_category_slider_init','init',['id' => $this->instance->id, 'category' => $this->config->alt_top_category, 'current_view' => 1]);
        }

        $this->page->requires->js_call_amd('block_mycourse/compact_view','init',['id' => $this->instance->id]);
        $this->page->requires->js_call_amd('block_mycourse/init_load','init',['id' => $this->instance->id]);
    ...
    public function get_content(): object {
        ...
        if(!empty($this->config->hide_if_empty)) {
            $builder = new block_mycourse\builder($this->config,$this->instance->id);
        ...
        $this->content->text = $OUTPUT->render_from_template('block_mycourse/wrapper', ['spinner' => \local_mscore_helper::get_spinner()]);
